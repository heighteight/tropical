...
Let $u,v: I\to \Lawv$ and suppose $|u(i)-v(i)|\leq \delta$, for all $i\in I$.
...
Let $f: [0,\infty)^{X}\to \Lawv$ be concave, monotone increasing, and continuous.  
...
For all maps $f,g: \Lawv^{X}\to \Lawv^{Y}$,  
$
...
For all $f: \Lawv^{X}\to\Lawv^{Y}$, let $\delta^{(n)}f:(\Lawv^{X})^{n}\to \Lawv^{Y}$ indicate the $n$-linear function given by 
$$
...
Notice that $\widehat{\delta^{(n)}f}\in \Lawv^{X^{n}\times Y}$ satisfies 
