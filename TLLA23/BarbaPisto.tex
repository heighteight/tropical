...
%hence, one is usually interested to know whether the computed function is \emph{not too far} from $f$, rather than equal to $f$.
...
Furthermore, $!$ can be decomposed into a family of \emph{graded} exponentials $(!_n)_{n\in\N}$ turning $(\LREL,(!_n)_{n\in\N})$ in a model for a %$\mathit{BLL}$-style
simply typed $\lambda$-calculus with \emph{bounded} duplications, on the style of~\cite{Brunel2014}, call it $\BSTLC$.%(see e.g.~\cite{Brunel2014}).

 
Finally, $\LREL_!$ can be equipped with a \emph{differential operator} $D:\HOM{\LREL}{!X}{Y}\to \HOM{\LREL}{!(X+X)}{Y}$ defined by: $(Dt)_{\mu\oplus\rho,b}:=t_{\rho+\mu,b}$ if $\#\mu=1$ and $:=\infty$ otherwise. %(where a multiset $\nu \in !(X+X)$ is identified with a disjoint sum of $\mu,\rho\in !X$).
...
The proof of the result above requires the study of %topological and metric properties of
...
If a tLs $f: \Lawv^{X}\to \Lawv^{Y}$ arises from a %bounded
...
%\end{proof}
The result above suggests that, far from $0$, the behavior of tLs with finitely many variables can be studied with the tools of tropical geometry (e.g.~tropical roots, Newton polygones). %as suggested in the next section. 
...
This idea suggests several natural directions in which the tropical semantics of a higher-order programs could be used to deduce properties which can be expressed as an optimization problem.%: for a $\lambda$-term $M$, which can be expressed in $\LREL_!$ as a tropical polynomial or power series $\model M$, and we investigate what quantitative properties of $M$ can be deduced from it. In particular, it is useful to think of $\model M = \trop f$ for some polynomial or power series $f$.
...
A quantale module over $\Lawv$, shortly a \emph{$\Lawv$-module}%(not to be confused with a module on $\Lawv$ \emph{as a semiring})
...
%Notice that this defines the usual notion of module over the Lawvere quantale $\Lawv$, and this does \emph{not} coincide with the modules on $\Lawv$ \emph{as a semiring} (which are indeed well-studied and well-known to provide a $CC\partial C$ {\color{red}LEMAYYYY}). 
...
%While $\LREL$ corresponds to considering modules (the $\LREL^X$'s) whose vectors are given in coordinates w.r.t.\ a \emph{fixed base} (the set $X$), $\Lawv\SF{Mod}$ corresponds to considering modules in abstract, without fixing a base.
...
%
%This is interesting for us because it suggests to transport the metric point of view of $\GMet$ onto our modules in $\Mod$.
...
\end{theorem}
%In order to ,, consisting in defining a symmetric algebra %$\Sym_{n}(M)$
