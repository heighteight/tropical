...
\end{theorem}
That is, one does not need to stick to a base in order to interpret types/programs.
This differential extends the one for $\LREL_{!}$;
in order to define the exponential $!$ over $\Mod$ we use the well-known recipe from \cite{Mellies2018, Manzo2013}, consisting in defining a symmetric algebra $\Sym_{n}(M)$ as the equalizer of all permutative actions on $n$-tensors $M\otimes \dots \otimes M$, and then define $!$ as the product of the symmetric algebras.
