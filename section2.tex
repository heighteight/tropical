...
At the basis of our approach is the observation that the \emph{tropical semiring} $([0,\infty], \min, +)$, which is at the heart of tropical mathematics, coincides with the \emph{Lawvere quantale} $\Lawv$ \cite{}, which is at the heart of the categorical study of metric spaces initiated by Lawvere himself \cite{}.
Let us recall that a quantale is a complete lattice endowed with a continuous monoid action. In our case we have $\Lawv=([0,\infty], \geq,+)$. Observe that the lattice join operation of $\Lawv$ coincides with the idempotent semiring operation $\min$. 
In particular, as discussed in Section \ref{section3}, tropical linear algebra coincides with the study of matrices with values taken over $\Lawv$.
At the same time, a (pseudo-)metric is a function $d:X\times X\to \Lawv$ satisfying usual axioms, e.g.~the triangular law (indeed, such $d$ can be seen as a $\Lawv$-\emph{enriched category}, see Section \ref{section6}). 


A \emph{tropical polynomial} is a piecewise linear function $\varphi:\Lawv\to \Lawv$ of the form 
\begin{align}\label{eq:polytrop}
\varphi(\alpha)= \min_{i=1,\dots,k}\left\{ i\alpha + c_{i}\right\}
\end{align}
with coefficients $c_{i}$ taken from $\Lawv$. A value $\alpha\in \Lawv$ is a \emph{root} of the polynomial $P$ when
...
In fact, this intuition can be made precise as follows: for any positive real $t$, the tropical polynomials are in one to one correspondence with the functions $f:[0,1]\to [0,1]$ which can be written as a \emph{parameterized} polynomial 
...
More generally, for any function $f:[0,1]\to [0,1]$ which can be written as a (parameterized) \emph{power series} of the form $f(x)= \sum_{n}t^{c_{n}}x^{n}$ (such functions arise naturally from the interpretation of probabilistic programs, as we'll recall in Section \ref{section5}), we can define its \emph{tropicalization} $\trop f: \Lawv \to \Lawv$
as the function 
\begin{align}
\trop f(\alpha) =\inf_{n}\left\{ i\alpha+c_{i}\right\}
\end{align}
Such functions, called \emph{tropical Laurent series} \cite{}, will be discussed in more detail in Section \ref{section5}.

The functions $\varphi$ and $f$ can be related by a limit passage as follows: the functions $\phi_{t}(x)= -\log_{t}x$ and $\varphi_{t}(\alpha)= t^{-\alpha}$ define continuous bijections between $[0,1]$ and $[0,\infty]$ and, by letting
$\varphi_{t}: [0,\infty]\to [0,\infty]$ be defined by 
$\varphi_{t}(\alpha)= \phi_{t}\circ f \circ \psi_{t}$, one has that 
$\varphi= \lim_{t\to 0}\varphi_{t}$. 
