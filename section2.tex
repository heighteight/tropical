...
In the following sections we will sometimes make reference to a basic type system, that we call $\BSTLC$, for bounded higher-order programs, with types defined via $A::= o \  \mid \  (!_{\leq n}A) \multimap A  $, 
where $n\in \BB N$. Intuitively, $!_{\leq n}A\multimap B$ is the type of functions from $A$ to $B$ that may use their input at most $n$ times. More details about $\BSTLC$ are provided in the Appendix.
