...
A natural way to account for these situations is to consider forms of denotational semantics in which types are endowed with a \emph{behavioral metric}, that is, a metric on programs which measures differences of behavior. 
...
\C C(X,Y)\to \C C(X\times X,Y)$ satisfying some axioms which capture usual properties of differentials (e.g.~the linearity of $D$ in one of its two variables, the chain rule,  Schwarz's theorem etc.).
\end{itemize}
...
\subsection{{\color{red}Spoiler section ?}}%Outline of the paper}
