...
\subsection{Log-probabilities with a worked out example}
...
{\small{\[\begin{array}{cccc}
           ...
           M\oplus_p N \to (1-p)N
          \end{array}\]}}
%We add reduction rules:$ M\oplus_p N \to pM \quad$ and $\quad M\oplus_p N \to (1-p)N,$
...
\begin{remark}
 ...
 For instance, $\model{\vdash_{\STLC_\oplus} \true:\bool}_i$ is $p$ if $i=1$ and it is $1-p$ if $i=0$.
 In general, one can show that $\model{\Gamma \vdash_{\STLC_\oplus} M:A}\in[0,1]^{!\model \Gamma \times \model A}$.
 Coefficients $> 1$ may be considered when dealing with other kinds of effects.
\end{remark}
