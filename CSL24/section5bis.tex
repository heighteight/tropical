...
More abstractly, it is well-known that the category $\Met$ of pseudo-metric spaces and non-expansive maps, is \emph{not} cartesian closed, so it is not a model of $\STLC$ (yet, several cartesian closed \emph{sub-}categories of $\Met$ do exist, see e.g.~\cite{Clementino2006, PistoneFSCD2022}).
