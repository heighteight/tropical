...
Actually, we will work with the so-called \emph{qualitative} Taylor expansion, where one quotients for idempotency of the $+$ operator in the series, and thus the Taylor expansion is just a set (see \autoref{sec:TayLip}).% (this perspective is made clearer by the related approach of the \emph{resource $\lambda$-calculus} \cite{}).
...
\subparagraph*{Tropical Mathematics: a possible synthesis}%Outline of the paper}
...
Yet, what if these polynomials were tropical ones, i.e.~piecewise linear functions? This way, the Taylor formula could really be interpreted as a decomposition of $\lambda$-terms via limits (indeed, $\inf$s) of Lipschitz maps. In other words, unbounded term application could be seen 
