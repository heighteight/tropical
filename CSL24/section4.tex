...
In all this section we consider the sets $\Lawv^X$ endowed with the usual sup-norm $\norm{x}_\infty:=\sup_{a\in X} x_a$.
...
\subparagraph*{Continuity of tLs}\label{subsec:cont}%$\Lawv^{X}$ as a normed cone}
...
\subparagraph*{Lipschitz-continuity of tLs}\label{sec:4C}%$\Lawv^{X}$ as a metric space.}
...
One could now obtain the locally Lipschitz property of also infinitely many variables tLs from the results in \cite{Cobzas2017}.
In \cite[Theorem 6.4]{Cobzas2017} the locally Lipschitz property is obtained in general for LCTVS under the hypothesis of continuity.
Instead, we replace the continuity hypothesis by the locally bounded condition in our next theorem, which corresponds to the one considered in \cite[Property 6.5]{Cobzas2017}.
Our next theorem is basically a particular case of \cite[Theorem 6.9, Lemma 6.10]{Cobzas2017}, but we prefer instead to state it for a less general case and not talk about more complex argument involving LCTVS.

\begin{theorem}\label{thm:locLip}
 ...
 Moreover, the Lipschitz constant of $f_{\big|_{\overline{B_{\delta}(x)}}}$ can be chosen to be $\frac{1}{\delta}\max_{\overline{B_{3\delta}(x)}} \absv f$.
\end{theorem}
\begin{proof}[Proof]
...
Now since $(0,\infty)^X$ is open and convex in $\BB R^X$, %and all tLs are non-negative, 
we immediately have:

\begin{theorem}\label{thmTLSlocLip}
 All tLs $f:\Lawv^X\to\Lawv$ are locally Lipschitz on $(0,\infty)^X$.
 Moreover, the Lipschitz constant of $f_{\big|_{\overline{B_{\delta}(x)}}}$ can be chosen to be $\frac{1}{\delta}\max_{\overline{B_{3\delta}(x)}} f$.
\end{theorem}

\subparagraph*{Tropical differential of tLs}

The differential operator $D$ of $\LREL_{!}$ translates into a differential operator $D_{!}$ turing a tLs $f:\Lawv^{X}\to \Lawv^{Y}$ into a tLs $D_{!}f:\Lawv^{X}\times \Lawv^{X}\to \Lawv^{Y}$, linear in its first variable, and given by 
\begin{equation}
