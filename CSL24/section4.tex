...
We conclude this subsection by noticing that, as a set, $\Lawv=[0,\infty]$, and  $[0,\infty]^X$ with the usual $+$ and $\cdot$ is a $\BB R_{\geq0}$-semimodule, let us call it $\overline{\BB R}_{\geq 0}^X$.
Together with the norm $\norm{\cdot}_\infty$, it can be showed to be a Scott-complete \emph{normed cone} (see~\cite{Selinger2004} or the appendix).
...
Such order actually coincides with the pointwise order on $\overline{\BB R}_{\geq 0}$.
...
\begin{enumerate}
\item If a tps $f: \Lawv^{X}\to \Lawv^{Y}$ arises from a matrix $\matr f:X\times Y\to \Lawv$ (i.e.~it is tropical linear), then $f$ is non-expansive (i.e.\ $1$-Lipschitz).  
