...
in analogy with that happens in usual metric semantics, we have:


\begin{proposition}\label{prop:troplinear}
All tropical \emph{linear} functions $f: \Lawv^{X}\to \Lawv^{Y}$ are non-expansive (i.e.\ $1$-Lipschitz).  
\end{proposition}
%\begin{proof}[Proof sketch]
...
As consequence of Theorem~\ref{theorem:fepsilon}, the tLs with \emph{finitely many} variables are always \emph{locally} Lipschitz on all $(0,\infty)^X$.
%Actually, we can prove a more general statement, also covering the infinitary case.

We wish now to move to the case of infinitely many variables.
...
Instead of showing how our case fits into such theorems, we prefer to state a theorem, which is basically a particular case of \cite[Theorem 6.9, Lemma 6.10]{Cobzas2017}, already fitting in the topics under consideration in this paper.
...
Therefore, since $(0,\infty)^X$ is open and convex in $(\BB R^X,\norm\cdot)$, %and all tLs are non-negative, 
we immediately have that all tLs $f:\Lawv^X\to\Lawv$ are locally Lipschitz on $(0,\infty)^X$.
