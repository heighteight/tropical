...
For a power series $f(x)=\sum_{i\in I} a_i x^i$ (polynomials being the case for $I$ finite) defined on $[0,1]$ and coefficients $a_i$ in $[0,1]$, %and for a fixed valuation $\mathrm{val}:Q\to \Lawv$, 
one defines the \emph{tropicalisation} $\trop f:\Lawv \to \Lawv$ of $f$ as the tropical polynomial/power series function $\trop f(\alpha):=\inf_{i\in I} \set{-\log(a_i)+i\alpha}$.
