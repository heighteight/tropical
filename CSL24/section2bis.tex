...
Remember that a valuation by definition always sends $1\in Q$ to $0$.
Then one defines the \emph{tropicalisation} $\trop^{\mathrm{val}}f:\Lawv \to \Lawv$ of $f$ as $\trop^{\mathrm{val}}f(\alpha):=\inf_{i\in I} \set{\mathrm{val}(a_i)+i\alpha}$.
The constantly equal to $0$ valuation is called the \emph{null valuation}.
A common valuation on $Q=[0,1]$ is $\mathrm{val}(a)=-\log a$.
\end{remark}
