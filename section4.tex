...
Now \mbox{$\C F_\epsilon:=\set{n\in\N \mid
\widehat f(n)<\infty \textit{ and } 
\widehat f(m)> \widehat f(n)+\epsilon \textit{ for all } m<n}$}
does the job.
\end{proof}
...
\end{theorem}
\begin{proof}
 It follows after adaptating [Theorem , \cite{}] in order to prove that if a real-valued function on a locally convex topological $\BB R$-vector space is, locally around $x$, concave and bounded by a finite constant, then it is continuous at $x$.
\end{proof}
