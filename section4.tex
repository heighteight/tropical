...
%many properties of these functions still have to be well-understood. 
In this section we establish some topological and metric properties of tropical Laurent series, this way highlighting the rich and interesting topological and metric structure of the category $\LREL_{!}$.
...
\caption{
%\ref{fig:plot1}
 (a) Plot of the tropical polynomials $\varphi_{n}$, for $n\leq 4$ (from top to bottom), and of their limit tLs $\varphi$ (in violet). The points where the slope changes are  the tropical roots of $\varphi$, i.e.~the points $x=2^{-(i+1)}$, satisfying $ix+2^{-i}=(i+1)x+2^{-(i+1)}$.
%\ref{fig:plot2}
(b) Plot of
...
\begin{theorem}\label{thm:ScottCont}
 All tLs $\Lawv^X\to\Lawv^Y$ are Scott-continuous on $\BB R_{>0}^X$, w.r.t.\ the cone orders.
...
Observe that, for any tropical polynomial $\varphi:\Lawv^{X}\to \Lawv^{Y}$, the associated matrix has shape $!_{\mathrm{deg}(\varphi)}(X)\times Y\to \Lawv$ (as any monomial $ix+c_{i}$ yields a matrix entry on $!_{i}X\times Y$). Hence, using Proposition \ref{prop:boundedlip}, we have:
