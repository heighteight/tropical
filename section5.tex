...
Answering 1) and 2) amounts at solving a maximization problem related to $P_{\omega}$, which is more easily solved by 
...
\begin{equation}
  ...
  \end{array}
\end{equation}
In both cases, passing to $\trop P_{\omega}$ makes the problem easier, as this may be reduced to computing the tropical roots of the polynomial (which can be done in linear time \cite{Noferini2015}). %the tropicalisation operator $\trop{}$ as well as the \emph{negative $\log$-probabilities} appear.
