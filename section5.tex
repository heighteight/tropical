...
Thinking, as we will, to $p,q$ as parameters, it can thus also be read as the \emph{likelihood function} of such event.
...
\begin{remark}
 By adapting [Section IV, Section VI, \cite{Manzo2013}], it can be seen that $\LREL_!$ is a model of $\STLC_\oplus$.
...
Therefore, assuming the interpretation of a term known and assuming the tropical polynomials (in $p$ and $q$, not after the substitution $q:=1-p$) are extracted from it, one could study the optimisation problems \ref{eq:argmax}, \ref{eq:max} \emph{without} having to reduce the term.
...
\begin{remark}
 Taking the usual $M$ as example, the $p\in[0,1]$ s.t.\ $(p,1-p)$ is a tropical root of $\trop Q_1$ or of $\trop Q_0$ give, by definition of tropical root, the values of the probabilistic parameter in $\oplus_p$ for which there are at least two different equiprobable paths of samples from $M$ to its normal form.
