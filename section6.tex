...
FOLLOW SECTION III.C for definitions of models of LL and DiLL.
...
Also in this case, the construction above generalizes the situation in $\LREL$: for the modules of the form $\Lawv^{X}$, the symmetric algebra construction coincides with the usual one, provided by multisets.
\begin{proposition}
For any set $X$, 
$!_{n}(\Lawv^{X})\simeq \Lawv^{\C M_{n}(X)}$, where
$\C M_{n}(X)$ is the set of multisets of $X$ of cardinality $n$. 
\end{proposition}



At this point, we can define the exponential comonad as 
$!M:= \prod_{n\in \BB N}!_{n}M$, obtaining a Lafont category.
ECC....
